/**
 * run_swba_estimator.cpp
 * 
 * Command-line executable for running C++ SWBA estimator on simulation data.
 * Designed to integrate with Python pipeline (tools/cli.py e2e/e2e-simple).
 * 
 * Usage:
 *   run_swba_estimator --input simulation.json --output output_dir/ [--config config.yaml]
 */
...
public:
    ...
    ProcessedVisualFrame process_frame(
        ...
        // Note: CameraFrame doesn't have frame_id, using timestamp-based ID
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "=== C++ SWBA Estimator ===" << std::endl;
    ...
    config.use_ideal_coordinates = args.use_ideal_coordinates;
    ...
    // Get all keyframes
    std::vector<CameraFrame> keyframes;
    for (const auto& frame : sim_data.camera_frames) {
        if (frame.is_keyframe) {
            keyframes.push_back(frame);
        }
    }
    
    // Process preintegrated IMU and matching keyframes
    size_t kf_idx = 0;
    ...
    for (size_t i = 0; i < sim_data.preintegrated_imu.size(); ++i) {
        const auto& imu_data = sim_data.preintegrated_imu[i];
        ...
        cumulative_time += imu_data.dt;
        
        // Process at most ONE keyframe per IMU segment (matching Python behavior)
        if (kf_idx < keyframes.size() && keyframes[kf_idx].timestamp <= cumulative_time + 0.01) {
            const auto& keyframe = keyframes[kf_idx];
            
            if (args.verbose) {
                std::cout << "  Processing keyframe " << kf_idx << " at t=" << keyframe.timestamp << std::endl;
            }
            
            if (true) {  // Process the keyframe
                ...
                auto processed_frame = preprocessor.process_frame(
                    keyframe, current_pos, current_rot, landmarks_map
                );
                ...
                if (args.verbose) {
                    std::cout << "  Keyframe " << kf_idx << " at t=" << keyframe.timestamp
        ...
        // Show progress
        if ((i + 1) % 10 == 0 || args.verbose) {
            std::cout << "  Processed " << (i + 1) << "/" << sim_data.preintegrated_imu.size() 
    ...
    result.iterations = 0;  // Total iterations across all updates
    ...
    // Save result
    std::string output_file = args.output_dir + "/cpp_swba_result.json";
